...
create policy ref_read on public.feature_flag for select to authenticated
  using ((select private.my_scope()) is not null);
create policy ref_read on public.department for select to authenticated
  using ((select private.my_scope()) is not null);
create policy ref_read on public.position_role for select to authenticated
  using ((select private.my_scope()) is not null);
create policy ref_read on public.status_option for select to authenticated
  using ((select private.my_scope()) is not null);

-- MEMBER: โครงการ = ทั้งหมด, ฝ่าย = ฝ่ายตน, สมาชิก = ตัวเอง
create policy member_read on public.member for select to authenticated
  using (
...
create policy user_account_read on public.user_account for select to authenticated
...
create policy letter_read on public.letter for select to authenticated
  using ((select private.can_read_dept('LETTER', department)));
create policy budget_read on public.budget for select to authenticated
  using ((select private.can_read_dept('BUDGET', department)));
create policy risk_read on public.risk_issue for select to authenticated
...
create policy registration_read on public.registration for select to authenticated
  using (
...
create policy evidence_review_read on public.evidence_review for select to authenticated
  using (
    (select private.flag_is('READ:EVIDENCE', 'ENABLED_READ_ONLY'))
    and (select private.my_scope()) = 'PROJECT'
  );
create policy evaluation_question_read on public.evaluation_question for select to authenticated
  using (
    (select private.flag_is('READ:EVALUATION', 'ENABLED_READ_ONLY'))
    and (select private.my_scope()) = 'PROJECT'
  );
create policy evaluation_response_read on public.evaluation_response for select to authenticated
  using (
    (select private.flag_is('READ:EVALUATION', 'ENABLED_READ_ONLY'))
    and (select private.my_scope()) = 'PROJECT'
  );
...
grant execute on function
  ...
  private.role_scope(public.app_role), private.current_ctx(),
